...
class SlashCommandPopup(Container):
    def __init__(self, text_area: ChatInputArea):
        super().__init__()
        self.text_area = text_area
        self.items: List[DropdownItem] = []
        self.selected_index = 0
        self.filtered_items = self._get_files()
    ...
    def compose(self) -> ComposeResult:
        ...
        self.update_items("")
        yield Container(id="items-container")
    ...
    def update_display(self):
        ...
        if "/file" in current_line:
            filter_text = current_line[current_line.index("/file")+5:].strip()
            self.update_items(filter_text)

    def on_key(self, event):
        if event.key == "escape":
            self.remove()
            self.text_area.styles.height = self.text_area.height
    ...
    def confirm_selection(self) -> None:
        ...
        selected_text = selected.content  # or selected.get_content()
